    ...
    <body>
        <main>
            ...
            if (isset($_POST['submit'])) {
                ...
                if ($firstName !== '' || $lastName !== '' || $userName !== '' || $password !== '' || $email !== '' || $contact !== '') {
                    $sql = "SELECT * FROM ncs_user WHERE uname='$userName' AND upass='$password'";
                    $queryResult = $conn->query($sql);
                    $check_result = $queryResult->num_rows;
                    if ($check_result == 1) {
                        echo '<div class="container w-full mx-auto flex justify-center"><h3>User/password already exists or empty </h3></div>';
                    } else {
                        /* File Upload Section */
                    ...
                    echo '<div class="container w-full mx-auto flex justify-center"><h3>Input Field is empty!!!</h3></div>';
                }
            }
            ?>
            ...
            <section class="container w-full mx-auto">
                <form class="form-control space-y-6 p-6" action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST"
                    ...
                    <div class="grid grid-cols-2 items-center justify-center gap-6">
                        ...
                        <div>
                            <input type="submit" name="submit" value="Create User" id=""
